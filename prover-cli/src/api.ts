...
export const getProverLedgerState = async (
  ...
  const state = await providers.publicDataProvider
    ...
    .then((contractState) => (contractState != null ? contractState.data : null));
...
export const joinContract = async (
  ...
  const proverContract = await findDeployedContract(providers, {
    contractAddress,
    contract: proverContractInstance,
    privateStateId: 'proverPrivateState',
    initialPrivateState: { proofs: [], status: 'idle' },
  });
  logger.info(`Joined contract at address: ${proverContract.deployTxData.public.contractAddress}`);
...
export const deploy = async (
  ...
  logger.info(`Deployed contract at address: ${proverContract.deployTxData.public.contractAddress}`);
  return proverContract;
};

// Salary proof request type
...
export const submitProofRequest = async (
  proverContract: DeployedProverContract,
  proofRequest: SalaryProofRequest,
...
// This prover CLI only handles proof generation

export const displayProofStatus = async (
  providers: ProverProviders,
  proverContract: DeployedProverContract,
): Promise<{ proofStatus: any; contractAddress: string }> => {
  const contractAddress = proverContract.deployTxData.public.contractAddress;
  const proofStatus = await getProverLedgerState(providers, contractAddress);
  if (proofStatus === null) {
    logger.info(`There is no prover contract deployed at ${contractAddress}.`);
  } else {
    logger.info(`Current proof status: ${JSON.stringify(proofStatus)}`);
  }
  return { contractAddress, proofStatus };
};

export const findExistingContract = async (
  providers: ProverProviders,
): Promise<DeployedProverContract | null> => {
  try {
    logger.info('Searching for existing prover contracts...');
    
    // This is a placeholder implementation
    // In a real implementation, you would:
    // 1. Query the indexer for contracts deployed by the current wallet
    // 2. Filter for prover contracts
    // 3. Return the most recent/active one
    
    // For now, we'll return null to always deploy new contracts
    // You'll need to implement the actual contract discovery logic
    // based on your blockchain's indexer capabilities
    
    const walletAddress = providers.walletProvider.coinPublicKey;
    logger.debug(`Searching for contracts deployed by wallet: ${walletAddress}`);
    
    // Placeholder: return null for now (always deploy new)
    return null;
  } catch (error) {
    logger.debug(`Error finding existing contracts: ${error}`);
    return null;
  }
};
...
export const configureProviders = async (wallet: Wallet & Resource, config: Config) => {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider<typeof ProverPrivateStateId>({
      privateStateStoreName: contractConfig.privateStateStoreName,
    }),
    publicDataProvider: indexerPublicDataProvider(config.indexer, config.indexerWS),
    zkConfigProvider: new NodeZkConfigProvider<'submitProofRequest' | 'generateProof'>(contractConfig.zkConfigPath),
